...
public class D_Product {
    ...
    private void loadDataFromDatabase() {
        ...
        String query = "SELECT r.*, c.complainant_FName, c.complainant_LName, comp.compt_Subject, comp.compt_Category, comp.compt_CreatedDate, comp.compt_Desc, comp.compt_OrderID, " +
                "(SELECT COUNT(*) FROM resolution WHERE resolution_Status = 'In Progress' AND dept_ID = 3) AS inProgressCount, " +
                "(SELECT COUNT(*) FROM resolution WHERE resolution_Status = 'Waiting' AND dept_ID = 3) AS waitingCount, " +
                "(SELECT COUNT(*) FROM resolution WHERE resolution_Status = 'Resolved' AND dept_ID = 3) AS resolvedCount, " +
                "(SELECT COUNT(*) FROM resolution WHERE resolution_Status <> 'Resolved' AND dept_ID = 3) AS totalTickets " +
                "FROM resolution r " +
